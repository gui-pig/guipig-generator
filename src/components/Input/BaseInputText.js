...
class BaseInputText extends Component {
  ...
  resetInput = (val) => {
    ...
    else this.setState({ isValid: true, value: val });
  };
  ...
  onChange = (e) => {
    ...
    this.setState(
      { value: tempValue, validationText: this.validationText },
      () => setTimeout(() => this.setState({ isValid: true }), 4000)
    );
  ...
  render() {
    ...
    if (
      this.props.validationColor &&
      typeof validationStyle.color === "undefined"
    )
      validationStyle = Object.assign(validationStyle, {
        color: this.props.validationColor,
      });
    return (
      <FormGroup
        ...
        controlId="1"
      >
        {typeof this.props.labelText === "undefined" ? null : (
          <ControlLabel style={{ textAlign: "left !important" }}>
            {this.props.labelText}
          </ControlLabel>
        )}
        <InputGroup>
          {this.inputPrefix === "" ? null : (
            <InputGroup.Addon>{this.inputPrefix}</InputGroup.Addon>
          )}
          <FormControl
            ...
            style={{
              textAlign: this.validationType === "number" ? "right" : "left",
            }}
            type="text"
          ...
          {this.inputSuffix === "" ? null : (
            <InputGroup.Addon>{this.inputSuffix}</InputGroup.Addon>
          )}
        </InputGroup>
        {this.state.isValid ? null : (
          <HelpBlock style={validationStyle}>
